...
class Statistics {
  ...
  private setStats(statType: number) {
    ...
    if (!this.userStats.length) {
      const statsText = new BaseElement('h3', ['stats__h3']).element;
      const statsInfoWrapper = new BaseElement('div', ['stats__info-wrapper']).element;
      statsText.textContent = 'Недостаточно данных для статистики. Надо заниматься.';
      statsInfoWrapper.append(statsText);
    ...
    this.userStats.map((item) => {
      if (item.dates) {
        ...
        const statsInfoContainer = new BaseElement('div', ['stats__info-container']).element;
        ...
        switch (statType) {
          case 1:
            ...
            statsDate.innerHTML = `<span>${sDate}</span>`;
          ...
          case 3:
            ...
            statsHeader3.innerHTML = `Процент правильных ответов: <span>${percent.replace('.', ',')}</span>%`;
            break;
        }
        statsInfoContainer.append(statsDate, statsHeader1, statsHeader2, statsHeader3);
        statsInfoWrapper.append(statsInfoContainer);
        statsContainer.append(statsInfoWrapper);
      }
    });
  }
