...
class GamesLinks {
  ...
  constructor() {
    ...
    if (state.group < 6) {
      const groupButtonSprint = new Button(`Спринт`, ['group-btn', 'tutorial-games-link']).buttonElement;
      groupButtonSprint.dataset.level = '0';
      groupButtonSprint.addEventListener('click', this.handleLinksButtons);
      linksContainer.append(groupButtonSprint);
      const groupButtonAC = new Button(`Аудиовызов`, ['group-btn', 'tutorial-games-link']).buttonElement;
